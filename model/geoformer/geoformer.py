...
class GeoFormer(nn.Module):
    ...
    def forward(self, batch_input, epoch, training=True, show=False, vis_path=None):
        ...
        if training :
            ...
            batch_offsets_subsample = utils.get_batch_offsets(batch_idxs_[object_idxs_subsample], batch_size)
        ...
        if show:
            ...
            color[batch_input["labels"][batch_idxs == batch_id] > 3] = torch.tensor([1., 0., 0.]).float().cuda()
