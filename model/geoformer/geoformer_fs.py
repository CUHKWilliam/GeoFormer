...
class GeoFormerFS(nn.Module):
    ...
    def forward(self, support_dict, scene_dict, training=True, remember=False, support_embeddings=None,
        ...
        if remember:
            ...
            print("semantic_preds:", torch.unique(semantic_preds))
            fg_idxs = torch.nonzero(fg_condition).view(-1)
            if not fg_condition.any():
                return outputs
        ...
        if support_embeddings is None:
            support_embeddings = self.process_support(support_dict, training, vis_path=vis_path)  # batch x channel
        ...
        if show:
            ...
            coords_float_b_ = locs_float_[batch_idxs_ == batch_id]
            color = torch.ones((coords_float_b_.size(0), 3)).float().cuda()
            color[scene_dict["labels"][fg_condition][batch_idxs_ == batch_id] > 0] = torch.tensor([1., 0., 0.]).float().cuda()
            visualize_pts_open3d(coords_float_b_, color, filename=os.path.join(vis_path, "query_filtered.ply"))
