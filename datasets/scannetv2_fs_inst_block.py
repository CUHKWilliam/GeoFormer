...
class FSInstDataset:
    def __init__(self, split_set="train"):
        ...
        all_file_block_names = os.listdir(os.path.join(self.data_root, self.dataset, "scenes/blocks_bs3_s3/data"))

        ## TODO: scenes
        self.file_names = [
            os.path.join(self.data_root, self.dataset, f)
            for f in all_file_names
            if f.split(".")[0] in self.scan_names
        ]
        self.block_file_names = [
            os.path.join(self.data_root, self.dataset, "scenes/blocks_bs3_s3/data", f)
            for f in all_file_block_names
            if f.split(".")[0].split("_block")[0] in self.scan_names
        ]

        self.file_names = sorted(self.file_names)
        self.block_file_names = sorted(self.block_file_names)
    ...
    def testLoader(self):
        ...
        self.test_block_names = [os.path.basename(i).split(".")[0] for i in self.block_file_names]
    ...
    def testMergeFS(self, ids):
        ...
        block_idx = 0
        for i, test_block_name in enumerate(self.test_block_names):
            if query_scene_name in test_block_name:
                block_idx = i
                break
        query_scene_name = self.test_block_names[block_idx]
